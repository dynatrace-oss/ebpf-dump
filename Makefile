...
# Build environment, current options are local, github
ENV ?= local
include .env-$(ENV)
...
# IMAGE_TAG_BASE ?= research.dynatrace.com/ebpfdump
...
.PHONY: build
build: manifests generate generate-ebpf fmt vet ## Build manager binary.
...
docker-build: ## Build the operator docker image
	docker rmi manager ${IMG} &2>/dev/null || :
	docker build -t manager .
	docker tag manager ${IMG}
...
#	- $(CONTAINER_TOOL) buildx create --name ebpfdump-builder
#	$(CONTAINER_TOOL) buildx use epbf-dump-builder
#	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
#	- $(CONTAINER_TOOL) buildx rm ebpfdump-builder
#	rm Dockerfile.cross
...
deploy: manifests kustomize
	cd config/manager && $(KUSTOMIZE) edit set image ebpfdump-operator=${IMG}
...
delete-cluster-local: ## Delete the local cluster with kind
	kubectl config delete-cluster kind-ebpfdump || :
	kind delete cluster --name ebpfdump

##@ Other

.PHONY: kill-pods
kill-pods: ## Kill all pods and EbpfDump resources in cluster
	@NAMESPACE="ebpfdump-operator-system"; \
	NAME=$$(kubectl get pods -n $$NAMESPACE -o name); \
	kubectl delete $$NAME -n $$NAMESPACE
	kubectl delete EbpfDump --all --all-namespaces

.PHONY: docker
docker: ## Build and push the operator docker image
	@make docker-build
	@make docker-push
